...
sudo apt update && sudo apt install bat btop direnv eza fd-find fzf httpie jq neovim tealdeer ugrep unzip -y
...
wget -q -O "$LOCAL_DIR"/dysk https://dystroy.org/dysk/download/x86_64-linux/dysk
chmod +x "$LOCAL_DIR"/dysk
...
curl -s https://api.github.com/repos/fastfetch-cli/fastfetch/releases/latest | grep "browser_download_url" | cut -d '"' -f 4 | grep "amd64.deb" | xargs -n 1 curl -LJO
...
wget -q -O starship.sh https://starship.rs/install.sh
sh starship.sh -f -b "$HOME"/.local/bin
rm starship.sh
